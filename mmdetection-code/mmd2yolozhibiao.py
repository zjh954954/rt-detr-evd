...
def parse_opt():
    ...
    parser.add_argument('--label_coco', type=str, default='/root/val/annotations/val4.json', help='label coco path')
    #parser.add_argument('--pred_coco', type=str, default='runs/val/exp/predictions.json', help='pred coco path')
    parser.add_argument('--pred_coco', type=str, default='work_dirs/ssd300_evd/test/prediction.pickle', help='pred coco path')
...
if __name__ == '__main__':
    ...
    for idx, image_id in enumerate(tqdm.tqdm(list(image_id_hw_dict.keys()), desc="Cal mAP...")):
            # 添加错误处理，检查image_id是否在label_id_dict中
        if image_id not in label_id_dict:
            print(f"警告：跳过ID为{image_id}的图像，找不到对应标签")
            continue
